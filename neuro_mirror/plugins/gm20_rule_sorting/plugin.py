...
class Gm20RuleSortingPlugin(Plugin):
    ...
    def subscribed_topics(self) -> tuple[str, ...]:
        return (Topics.REQ_GM20_START, Topics.REQ_GM20_ANSWER)

    async def handle_event(self, event: Event) -> None:
        request_id = str(event.payload.get("_request_id") or "")
        if event.topic == Topics.REQ_GM20_START:
            payload = self._start_session()
            topic = Topics.RESP_GM20_START
        else:
            payload = self._check_answer(event.payload)
            topic = Topics.RESP_GM20_ANSWER
