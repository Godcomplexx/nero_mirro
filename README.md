...
- `video/NNNNNN.webm` — видео сессии, записанное браузером кусками по 5 секунд,
  и `video/index.jsonl` со смещением каждого куска от начала записи;
...
Куски видео по отдельности не воспроизводятся: заголовок лежит только в первом.
Чтобы получить проигрываемый файл, склейте их побайтово по возрастанию номера:

```powershell
Get-ChildItem runtime\dataset\<session_id>\video\*.webm | Sort-Object Name |
  ForEach-Object { [System.IO.File]::ReadAllBytes($_.FullName) } |
  Set-Content -Path session.webm -Encoding Byte
```
